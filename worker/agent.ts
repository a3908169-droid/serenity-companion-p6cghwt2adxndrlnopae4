...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onStart(): Promise<void> {
    ...
    console.log(`暖暖回声 Agent ${this.name} 已就绪，会话 ID: ${this.state.sessionId}`);
  }
      ...
      if (stream) {
        ...
        (async () => {
          try {
            ...
            try {
              const errorMessage = '抱歉，暖暖回声刚才断开了，请再说一次好吗？';
            ...
            } catch (writeError) { console.error('Error writing error message:', writeError); }
          } finally {
            try { writer.close(); } catch (closeError) { console.error('Error closing writer:', closeError); }
          }
        })();
